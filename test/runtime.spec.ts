...
describe('Testing runtime features', () => {
    ...
    test.each([0, 1, 2])('select tab updates DOM (active class)', (tabToSelectIndex) => {
        ...
        const tabData = getTabDataFromElement(tabs[tabToSelectIndex]);
    ...
    it('roving tabindex: select next/prev tab via API (same logic as ArrowRight/Left)', () => {
        ...
        tabController.selectTab(getTabDataFromElement(tabs[1])!);
        tabs[1].focus();
        expect(tabs[0].classList.contains('active')).not.toBeTruthy();
        expect(tabs[1].classList.contains('active')).toBeTruthy();
        expect(tabs[2].classList.contains('active')).not.toBeTruthy();

        tabController.selectTab(getTabDataFromElement(tabs[2])!);
        tabs[2].focus();
        expect(tabs[0].classList.contains('active')).not.toBeTruthy();
        expect(tabs[1].classList.contains('active')).not.toBeTruthy();
        expect(tabs[2].classList.contains('active')).toBeTruthy();

        tabController.selectTab(getTabDataFromElement(tabs[0])!);
        tabs[0].focus();
        expect(tabs[0].classList.contains('active')).toBeTruthy();
        expect(tabs[1].classList.contains('active')).not.toBeTruthy();
        expect(tabs[2].classList.contains('active')).not.toBeTruthy();
    ...
    test.each(['right', 'left'])(
        ...
        (_direction) => {
            const tab2Data = getTabDataFromElement(tabs[2]);
            const nested0Data = getTabDataFromElement(nestedTabs[0]);
            const nested1Data = getTabDataFromElement(nestedTabs[1]);
            expect(tab2Data).toBeTruthy();
            expect(nested0Data).toBeTruthy();
            expect(nested1Data).toBeTruthy();
