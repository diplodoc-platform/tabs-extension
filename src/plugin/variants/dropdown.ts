...
export const dropdown: TabsTokensGenerator = (
    ...
    const menuTokens = tabs.flatMap((tab) => {
        ...
        const isActive = String(tab === activeTab);

        menuListItemOpen.attrSet('class', TAB_CLASSNAME);
        menuListItemOpen.attrSet(TAB_DATA_ID, tabId);
        menuListItemOpen.attrSet(TAB_DATA_KEY, tabKey);
        menuListItemOpen.attrSet(TAB_ACTIVE_KEY, isActive);
        menuListItemOpen.attrSet('aria-selected', isActive);
